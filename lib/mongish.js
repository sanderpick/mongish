  ...
  (function insert() {
    this.insert(props, opts, _.bind(function (err, ins) {
      if (err && err.code === 11000) {
        ...
        var dups = [];
        var match = err.message.match(/index: ([^\s]+)/);
        if (match && match[1]) {
          var parts = match[1].replace('$', '').split('.')
          var keys = parts[parts.length - 1]
          if (keys) {
            dups = _.compact(_.map(keys.split('1'), function (s) {
              return _s.trim(s, '_');
            }));
          }
        }
    ...
    this._update(query, props, opts, _.bind(function (err, doc) {
      if (err && err.code === 11000) {
        ...
        if (match && match[1]) {
          ...
          var key;
          if (keys) {
            key = _.first(_.map(keys.split('1'), function (s) {
              return _s.trim(s, '_');
            }));
          }
          if (!_force || !key || !_force[key]) return cb(err);
          props.$set[key] += '-' + attempt;
          ++attempt;
